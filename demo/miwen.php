...
$configs = array(
    ...
    'list_url_regexes' => array(
        'https://ask\.imiker\.com/explore/sort_type-no_top__category-all__page-\d+'
    ),
    'content_url_regexes' => array(
        "https://ask\.imiker\.com/question/\d+",
    ),
    'max_try' => 1,
    'max_depth' => 5,
    'export' => array(
    ...
    'fields' => array(
        array(
            'name' => "title",
            'selector' => "//div[@class='container']//div[@class='aw-mod-head']/h1",
            //'selector' => "/html/body/center/div[@class='maintable'][4]/div[@class='spaceborder']/table/tbody/tr[@class='header']/td/text()",
            'required' => true,
        ),
        array(
            'name' => "description",
            'selector' => "//div[@class='container']//div[@class='ueditor-p-reset']//div[@class='aw-question-detail-txt']",
            'required' => true,
        ),
        array(
            'name' => 'category_id',
            'selector' => "//div[@class='crumbs']/a[2]",
        ),
        array(
            'name' => 'user_id',
            'selector' => "",
...
$spider->on_scan_page = function($page, $html, $spider){
    for($i = 13; $i < 999; $i ++){
        $url = "https://ask.imiker.com/explore/sort_type-no_top__category-all__page-" . $i;
        //$spider->add_url($url);
    }
};
...
$spider->on_extract_page = function($page, $fields){
    ...
    preg_match("#/question/(\d+)#", $page['url'], $match);
    if (isset($match[1])) {
        $fields['a_id'] = $match[1];
    }
    return $fields;
